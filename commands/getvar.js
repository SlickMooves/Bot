...
module.exports = {
    ...
    async execute(interaction) {
        ...
        fetch(`https://authentication.astroz.cc/api/seller/?sellerkey=${sellerkey}&type=getvar&user=${user}&var=${name}`)
        .then(res => res.json())
        .then(json => {
            if (json.success) {
                interaction.editReply({ embeds: [new Discord.MessageEmbed().setTitle(`User variable successfully retrieved`).addField('Variable data:', `\`${json['response']}\``).setColor("BLUE").setTimestamp()], })
            } else {
