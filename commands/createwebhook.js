...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if(isNaN(authed))
        {
            return interaction.editReply({ embeds: [new Discord.MessageEmbed().setTitle('Failure, non-numerical answer provided.').setColor("RED")], })
        }

        if (baseurl.includes("http://") || baseurl.includes("https://"))
        { } else {
            return interaction.editReply({ embeds: [new Discord.MessageEmbed().setTitle("Failure, Please Include `http:\/\/` or `https:\/\/` on webhook link").setColor("RED").setTimestamp().setFooter({ text: "KeyAuth Discord Bot" })],  })
        }
