...
module.exports = {
    ...
    async execute(interaction) {
        ...
        fetch(`https://authentication.astroz.cc/api/seller/?sellerkey=${sellerkey}&type=killall`)
        .then(res => res.json())
        .then(json => {
            if (json.success)
            {
                interaction.editReply({ embeds: [new Discord.MessageEmbed().setTitle("Successfully Killed All Sessions").setColor("GREEN").setTimestamp().setFooter({ text: "KeyAuth Discord Bot" })], })
            } else {
